...
def remove_voc(voc,threshold):
    ...
    return list(set(saved))


#Updating the data set
def update_data(sentences,voc):
    new_sentences = []
    for l in sentences:
        new_words = []
        for t in l.split():
            if t in voc:
                ...
                new_words.append('uuo') #Here we are replacing the words with less frequency with a placeholder
...
new_data_eng = update_data(lines_eng,new_vocab_eng)
for i in range(10):
...
new_data_dutch = update_data(lines_dutch,new_vocab_dutch)
